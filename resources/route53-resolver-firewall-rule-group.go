...
func ruleGroupsToAssociationIds(ctx context.Context, svc Route53ResolverAPI) (map[string][]*string, error) {
	...
	for {
		...
		for _, frgAssociation := range resp.FirewallRuleGroupAssociations {
			associationId := frgAssociation.Id
			if associationId != nil {
				frgId := *frgAssociation.FirewallRuleGroupId
...
func getFirewallRules(ctx context.Context, svc Route53ResolverAPI, firewallRuleGroupId *string) ([]*Route53ResolverFirewallRule, error) {
	...
	for {
		...
		for _, firewallRule := range resp.FirewallRules {

			rule := Route53ResolverFirewallRule{
				Name:                       firewallRule.Name,
				FirewallDomainListId:       firewallRule.FirewallDomainListId,
				FirewallThreatProtectionId: firewallRule.FirewallThreatProtectionId,
				Qtype:                      firewallRule.Qtype,
			}
